...
#   2. Remove trusted publisher configs that don't match the desired config.
...
#
# The GitHub repository owner/name are derived at runtime from
# `[workspace.package.repository]` in Cargo.toml. The workflow filename
# (`release.yml`) and environment (`release`) follow cargo-dist conventions.
#
# It authenticates with `CARGO_REGISTRY_TOKEN`, which must have the
# `publish-new` and `trusted-publishing` scopes.
#
# Crates tracked in `.known-crates` are assumed to be configured and are
# skipped unless `--force` is used.
#
# Usage:
#   CARGO_REGISTRY_TOKEN=<tok> uv run scripts/setup-crates-io-publish.py \
#       [--dry-run] [--force] [--quiet]
#
...
def parse_github_owner_repo(workspace_package: dict[str, object]) -> tuple[str, str]:
    """Derive (owner, repo) from `[workspace.package.repository]` (a GitHub URL)."""
    repository = workspace_package.get("repository")
    if not isinstance(repository, str) or not repository:
        raise RuntimeError(
            "workspace.package.repository is missing from Cargo.toml; "
            "set it to your GitHub repository URL"
        )
    parsed = urlparse(repository)
    if "github.com" not in parsed.netloc:
        raise RuntimeError(
            f"workspace.package.repository must be a github.com URL, got {repository!r}"
        )
    parts = [p for p in parsed.path.split("/") if p]
    if len(parts) < 2:
        raise RuntimeError(
            f"could not parse owner/name from repository URL {repository!r}"
        )
...
def publish_placeholder_crate(
    ...
    with tempfile.TemporaryDirectory(prefix=f"{crate_name}-placeholder-") as temp_dir:
        ...
        description = (
            f"This is a placeholder release for an internal component "
            f"crate of {facade_crate}"
        )
        ...
        (temp_path / "README.md").write_text(
            ...
            f"# {crate_name}\n\n"
            f"This crate is an internal component of "
            f"[{facade_crate}](https://crates.io/crates/{facade_crate}). "
            f"This placeholder version ({PLACEHOLDER_VERSION}) only exists to "
            "reserve the crate name and enable trusted publishing for future "
            "releases.\n"
        )
...
def create_trusted_publisher(
    client: httpx.Client, crate_name: str, owner: str, repo: str
) -> None:
...
def is_desired_config(config: dict[str, object], owner: str, repo: str) -> bool:
    """Return True if *config* matches the workflow this repo uses."""
    return (
        config.get("repository_owner") == owner
        and config.get("repository_name") == repo
...
def main() -> None:
    ...
    if not candidates:
        if not quiet:
            print(
                f"All {len(crates)} publishable crates are in .known-crates "
                "— nothing to do."
            )
        return
    ...
    for crate in candidates:
        ...
        desired_configs = [
            config for config in configs if is_desired_config(config, owner, repo)
        ]
        extra_desired_configs = desired_configs[1:]
        non_matching_configs = [
            config for config in configs if not is_desired_config(config, owner, repo)
        ]
        ...
        if needs_initial_publish:
            if version == PLACEHOLDER_VERSION:
                print(
                    f"{name}: workspace version matches placeholder version "
                    f"{PLACEHOLDER_VERSION}; bump the real crate version "
            ...
            if not publish_placeholder_crate(name, workspace_package, facade_crate):
        ...
        if needs_add_publisher:
            ...
            try:
                create_trusted_publisher(auth_client, name, owner, repo)
            except httpx.HTTPStatusError as exc:
                handle_trusted_publisher_error(exc)
